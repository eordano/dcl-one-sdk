...
use catalyrst_auth_chain::{AuthLink as CryptoAuthLink, AuthLinkType, EthAddress};

pub use catalyrst_auth_chain::MAX_AUTH_CHAIN_LINKS;
...
impl AuthChainError {
    ...
    pub fn http_status_and_message(&self) -> (u16, String) {
        if self.is_bad_request() {
            ...
            (401, format!("Unauthenticated: {self}"))
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn bad_requests_answer_400_with_the_detail() {
        let err = AuthChainError::MalformedChain {
            detail: "unexpected key".into(),
        };
        assert_eq!(
            err.http_status_and_message(),
            (400, "Invalid Auth Chain: unexpected key".to_string())
        );
    }

    #[test]
    fn other_failures_answer_401_unauthenticated() {
        assert_eq!(
            AuthChainError::ForbiddenSigner.http_status_and_message(),
            (
                401,
                "Unauthenticated: Access denied, invalid signer".to_string()
            )
        );
    }
