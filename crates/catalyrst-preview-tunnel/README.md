...
Deploying: one ws-capable nginx location on any https vhost is enough - everything, including the agent trunk at `/t/_connect`, lives under `/t/`. Ready vhost snippet + systemd unit template in the deployment tree: `config/nginx/conf.d/05-preview-tunnel.conf`, `systemd/preview-tunnel.service`. Load-bearing directives: `proxy_http_version 1.1` + Upgrade/Connection passthrough, `proxy_read_timeout 1h`, `proxy_buffering off`, `client_max_body_size 0`.
