...
world's content server, your `DCL_ONE_SDK_TARGET_SERVER`, or with neither the
...
The scene decides where it goes: a `worldConfiguration.name` goes to the
...
tried first in the public rotation for parcels — the same two destinations
an unmodified build already reaches, just named so a fork can change them.
Nothing is remembered between runs. A scene of
the wrong kind for its target (parcels at the worlds server, a World at a
Genesis catalyst) is refused before the build when the host is known, and by
the server otherwise. A `worldConfiguration` section that names no world is
