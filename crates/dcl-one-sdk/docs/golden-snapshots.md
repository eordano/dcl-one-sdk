...
`build::build` (the repo's first in-process caller), read the artifacts back off
disk, run `deploy::prepare` with `timestamp = 0`. The build's own stdout is
deliberately *not* captured: its step lines carry elapsed times (`Scene chunk
saved bin/scene.js (4.99 ms)`) and a timing must never reach a golden.
...
node is a hard dependency of this harness, and already one of `build` (the type
check, and the `@dcl/inspector` data-layer fallback for composites carrying
their own `jsonSchema`). Missing node fails the test unless
...
quietly working. (`~system/CommsApi` was found this way while building the
smart-item fixture.)

`CONSOLE(...)` is an assertion for the same reason, and closes the one hole exit
status cannot: the generated startup catches everything `main()` throws and
...
node's `process.memoryUsage()` is GC-timing dependent, so it would make a flaky
...
no bytes and no messages will not be caught.** The honest route later is an
opt-in `--features quickjs-golden` tier with a vendored WASM blob, never the
default suite.
...
generator, crdt encoder, SDK chunk) needs it, so the format leaves the prefix
free; a two-way fixture later needs no format change.
...
re-bundled. Comparability is structural: identical line names, so a human can
lay the two files side by side.
...
`<CARGO_TARGET_TMPDIR>/golden/<name>.actual` dump and the exact regeneration
command. For the same reason the gate refuses to run at all with
`UPDATE_GOLDEN` set, exactly as it already refuses `ALLOW_SKIPPED_INTEGRATION`.
...
`ENTRYPOINT … composites=0` on the `composite` and `smart-item` goldens is not a
bug: `entrypoint::write_all_composites` only inlines a composite found at the
scene ROOT (`main.composite`). A Creator Hub scene's `assets/scene/main.composite`
reaches the runtime through `main.crdt` instead, which is what the `MAIN_CRDT`
line and the `main.crdt:` messages pin.
...
3. `UPDATE_GOLDEN=1 cargo test -p dcl-one-sdk --test golden` and **read the
   diff**.
4. Raise the `dcl-one-sdk golden` floor in `tests-manifest.tsv` (upstream
   workspace root; like `update-goldens.sh` it is not in the published tree, so
   this step is for contributors working in the source checkout).
