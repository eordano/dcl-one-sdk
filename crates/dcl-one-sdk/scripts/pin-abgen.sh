...
# that carries the node: root and catalyrst directly, the reference deployment and bevy-explorer
...
# the reference deployment/scripts/check-abgen-pins.sh, the gate that fails when any copy
...
one=$(cd "$here/../../.." && pwd)
gate=$one/the reference deployment/scripts/check-abgen-pins.sh
