...
4. **Rewrite every code reference** to the moved paths — grep the whole source tree for the old folder names and update each hit, including string literals in JSON and `.composite` files. Per-API rewrite table: `{baseDir}/references/api-mapping.md` → "Asset paths (folder reorganization)".
...
- **`Animator.playSingleAnimation` silently no-ops on clips not in `Animator.states` (porters who built the Animator in code)**: it returns `false` and does nothing if `clipName` isn't already in the entity's `states` array (verified — `@dcl/ecs/dist-cjs/components/extended/Animator.js` lines 35-46). No warning. SDK6 hid this — `Animator.getClip(name)` auto-created the clip on first use, so SDK6 scenes routinely call `playAnimation('Appear')` without declaring `'Appear'` upfront, and a naive port of `getClip(name).play()` → `playSingleAnimation(entity, name)` silently fails. Fix: walk every `playAnimation` / `getClip` call in the SDK6 source, collect the full set of clip names per entity, and list them all in the `states` array at `Animator.create` time. For a port-friendly shim with a lazy `states.push`, see the wrapper in [[animations-tweens]] (PITFALL section). **This rule does not apply** to scenes whose Animator was written by the Creator Hub Inspector (which pre-populates `states[]` with every GLB clip) or by asset packs / smart items (which ship with `states` populated), nor to models with no `Animator` at all — those rely on the autoplay path in the next bullet.
- **Unnecessary serialization overhead when porting `anim.stop(); anim.play()` to a hand-rolled SDK7 helper called every frame**: applies when the helper mutates `Animator.states` (`s.playing`, `s.shouldReset`) directly AND runs from a per-frame caller — a system update, or an input/raycast callback that fires every tick the input is held. The animation will NOT freeze (the CRDT unchanged-mutable suppression layer drops identical writes), but `getMutableOrNull()` every tick costs a serialization + byte comparison each frame. **Fix**: track the last-applied clip per entity and short-circuit the helper when unchanged; on a `noLoop + revertToIdle` re-call with the same clip, only refresh the revert timer instead of re-mutating states; clear the tracked clip before re-applying the idle clip from the revert callback. **Does NOT apply** when the helper is called only on a state transition (one-shot `onPointerDown`, edge-triggered state machine), nor when you use `Animator.playSingleAnimation` from a transition rather than a per-frame caller. See the "Short-circuit clip-switch helpers" best practice in [[animations-tweens]] for the full helper pattern.
- **`GltfContainer` autoplays one clip from a multi-animation .glb when no `Animator` is attached**: SDK6 models stayed in their bind pose by default; SDK7 picks one clip and plays it. This is the *complement* to the previous bullet, not a contradiction: no `Animator` → the renderer autoplays; `Animator` present + `playSingleAnimation` → the clip must be in `states[]`. Which clip is autoplayed is not stable across models (observed: ghosts with no `Animator` autoplaying `die`). **Fix when you want a different default**: attach an `Animator.create(entity, { states: [{ clip: '<intended-default>', playing: true, loop: true }] })` to take control. **Fix when you also want to switch clips at runtime via `playSingleAnimation`**: list every clip you'll switch to in `states[]` (see previous bullet).
...
- **`@dcl/ecs-scene-utils` trigger areas (the "Utils library") AND hand-rolled per-frame AABB checks**: do NOT port the custom per-frame position-checking system, and do NOT recreate it from scratch in SDK7. Replace the whole pattern with the **native** `TriggerArea` component: `TriggerArea.setBox(entity)` (or `.setSphere`) + `triggerAreaEventsSystem.onTriggerEnter(entity, cb)` / `.onTriggerExit` / `.onTriggerStay`. **Behavior parity**: the Utils library only ever fired for the local player; native `TriggerArea` defaults to `ColliderLayer.CL_PLAYER`, which fires for ANY player on that layer. The correct guard for local-player-only behavior is `if (result.trigger?.entity !== engine.PlayerEntity) return` (NOT `result.triggeredEntity` — despite the confusing name, `triggeredEntity` is the trigger area's own entity, so comparing it to the player is always true and the guard never fires). Mapping table, sizing rules and gotchas: "Trigger areas" in `{baseDir}/references/api-mapping.md`; component reference: [[add-interactivity]].
- **`utils.Delay` / `utils.ExpireIn` from `decentraland-ecs-utils` (later `@dcl-sdk/utils`) AND hand-rolled per-frame timer systems**: do NOT port the community `Delay` component, do NOT recreate it in SDK7, and do NOT write a custom `setSceneTimeout` helper backed by a `timerSystem` that accumulates `dt`. Use the **`timers` named export from `@dcl/sdk/ecs`** — engine-bound `setTimeout` / `clearTimeout` / `setInterval` / `clearInterval`. Replace `entity.addComponent(new utils.Delay(2000, cb))` with `timers.setTimeout(cb, 2000)` and delete the "timer entity" the SDK6 scene created just to host the Delay component. **Argument order is `(callback, ms)`**, NOT `(ms, callback)` — the SDK6 `Delay(ms, cb)` order is the opposite of the JS standard, and custom helpers written with the SDK6 order are a known footgun. **Do NOT use the native JS `setTimeout` / `setInterval` globals** — although QuickJS exposes them, they are not bound to the scene engine and can introduce subtle problems; always go through `timers`. Full mapping: "Timers / delays" in `{baseDir}/references/api-mapping.md`; reference: [[scene-runtime]] (Timers section).
...
- **`Attachable.FIRST_PERSON_CAMERA` / `Attachable.AVATAR` on held items (guns, aiming reticles, flashlights, fixed-position tools)**: port these with `Transform.parent`, not `AvatarAttach`. Pick by intent: **`Transform.parent = engine.CameraEntity` is the right default for any aim-sensitive item** (gun, reticle, flashlight — anything pointed by looking around); it mirrors `FIRST_PERSON_CAMERA` and tracks yaw AND pitch. `Transform.parent = engine.PlayerEntity` is a common subtle failure mode — the player root is feet + body **yaw only**, no pitch, so the item stays flat when the player tilts the camera up to aim; use it only for body-fixed items that should NOT track aim (held shield, static torch, hip-level inventory). Reserve `AvatarAttach(anchorPointId)` for **cosmetic** items (hats, halos, backpacks, name plates): bone anchors inherit avatar animation (idle bob, walk cycle, gestures), so a gun on `AAPT_RIGHT_HAND` jitters every frame and is unaimable. Tables and anti-patterns: "Attaching items to the player" in `{baseDir}/references/api-mapping.md`; worked gun example: "Held items vs cosmetic items" in [[player-avatar]].
- **Billboarded models may appear facing AWAY from the camera after porting**: a model that showed its front face under SDK6's `new Billboard(false, true, false)` (Y-axis billboard) shows its **back** under SDK7's `Billboard.create(e, { billboardMode: BillboardMode.BM_Y })` with the same parent/child setup — the two SDKs disagree on which model face (+Z vs -Z) points at the camera. **The SDK7 `PBBillboard` interface has no `oppositeDirection` flag** — the only field is `billboardMode` (verified against `@dcl/ecs` `billboard.gen.d.ts`), so the fix is in the model's Transform, not the Billboard component. **Rotate the displayed model 180° on Y** (`Quaternion.fromEulerDegrees(0, 180, 0)`). If the entity is a parent (`Billboard`'d) + a child model, apply the 180° rotation to the **child**'s `Transform.rotation` — the parent entity's rotation is owned by the Billboard component and overwriting it on the billboarded entity has no effect. [UNVERIFIED whether this was an explicit SDK behavior change with a changelog entry, or a consequence of a separate convention change in the model importer — phrase any user-facing explanation as "the two SDKs disagree on billboard facing direction" rather than claiming a specific version introduced it.] See [[advanced-rendering]] (Billboard) for the SDK7 component reference.
- **UI pixel sizes do NOT carry over 1:1**: SDK6 UI (`UICanvas` / `UIImage` / `UIText`) was authored in raw pixels against a fixed screen size, so `width = 200` or `positionX = -350` were absolute. SDK7 React-ECS scales pixel values against a **virtual canvas**. Always set it explicitly on `ReactEcsRenderer.setUiRenderer`, using the pixel grid the SDK6 UI was drawn against (read the original `UICanvas`/`UIImage`/`UIText` setup; the legacy ECS lives at https://github.com/decentraland/ecs):
  ```ts
  ReactEcsRenderer.setUiRenderer(uiRoot, { virtualWidth: 1920, virtualHeight: 1080 })
  ```
  `1920x1080` is a reasonable default, but if the SDK6 scene assumed e.g. `1280x720`, use those numbers so existing pixel coordinates land in the same place. Version behaviors that move the grid (7.26.0+ per-platform default, mobile 16:9 override, `<= 0` disables scaling, no default below 7.26.0): "UI sizing — virtual canvas" in `{baseDir}/references/api-mapping.md`. See [[build-ui]] for the full React-ECS rewrite and its SDK version gate.
...
- **UI built with the SDK6 `UICanvas` / `UIImage` / `UIText` / `UIClickable`**: not API-compatible with SDK7. SDK7 UI is React-ECS (JSX) and this often requires a from-scratch rewrite — see [[build-ui]], plus the UI pixel sizing pitfall in **Common Pitfalls** for the virtual canvas above.
