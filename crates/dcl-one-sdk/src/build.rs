...
use crate::{entrypoint, esbuild, esbuild_service, scene::Project, split};
use anyhow::Result;
...
pub async fn build(opts: &BuildOptions) -> Result<Built> {
    ...
    if opts.split_sdk {
        return build_split(opts, project).await;
    }
    let main = project.main_output()?;
    let tsconfig = project.tsconfig()?;
    let outfile = project.root.join(&main);
    let mut steps = ux::Steps::new(if opts.skip_type_check { 1 } else { 2 });
    ...
    let es_opts = esbuild::EsbuildOptions {
        backend: opts.backend,
        production: opts.production,
    ...
    esbuild_service::run_with_fallback(&project, &es_opts).await?;
    tracing::info!("bundle saved {}", outfile.display());
    steps.done(format!(
        "Bundle saved {} ({})",
    ...
    crate::data_layer::regenerate_main_crdt(&project.root, opts.ignore_composite).await;
...
async fn build_split(opts: &BuildOptions, project: Project) -> Result<Built> {
    let main = project.main_output()?;
    let tsconfig = project.tsconfig()?;
    let outfile = project.root.join(&main);
    let (sdk_rel, scene_rel) = split::chunk_rel_paths(&main);
    let mut steps = ux::Steps::new(if opts.skip_type_check { 3 } else { 4 });
    ...
    let started = Instant::now();
    esbuild_service::run_with_fallback(&project, &sdk_opts).await?;
    ...
    let started = Instant::now();
    esbuild_service::run_with_fallback(&project, &scene_opts).await?;
