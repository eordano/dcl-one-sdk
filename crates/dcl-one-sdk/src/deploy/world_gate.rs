...
/// `world_at_genesis`, reached when `--target-content` /
/// `DCL_ONE_SDK_DEFAULT_TARGET` points a parcel scene at a worlds server.
/// Both the public server's wording and catalyrst-worlds' are recognised.
...
pub(super) fn refuse_plain_scene_at_worlds() -> anyhow::Error {
    UserError::new(
        ...
        TrySteps::one(
            "to deploy to Genesis City parcels: drop --target-content / DCL_ONE_SDK_DEFAULT_TARGET (and .dcl-one/deploy-target) so the scene routes to a Genesis catalyst",
        )
