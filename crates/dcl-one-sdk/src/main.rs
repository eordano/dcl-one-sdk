...
use dcl_one_sdk::esbuild::Backend;
use dcl_one_sdk::{
...
async fn run(command: Command) -> Result<()> {
    match command {
        Command::Init { dir, project, yes } => init::init(&init::InitOptions { dir, project, yes }),
        ...
        Command::Build {
            ...
            if watch {
                ...
                let chunk = if opts.split_sdk { 3 } else { 1 };
...
async fn watch_workspace(ws: &workspace::Workspace, opts: &build::BuildOptions) -> Result<()> {
    ...
    for (i, project) in ws.projects.iter().enumerate() {
        ...
        let chunk = if member.split_sdk { 3 } else { 1 };
