    ...
    let mut bundler = Bundler::new(bundler_options(project, opts)?).map_err(|e| {
        anyhow::Error::from(
            UserError::new(
                ...
                TrySteps::one("re-run with --backend esbuild")
                    .and("re-run with --verbose for the backend report"),
            )
            .why(format!("{e:?}")),
        )
    })?;
    let output = bundler.write().await.map_err(|e| {
        anyhow::Error::from(
            UserError::new(
                ...
                TrySteps::one("fix the errors above").and("or re-run with --backend esbuild"),
            )
            .why(format!("{e}")),
        )
    })?;
