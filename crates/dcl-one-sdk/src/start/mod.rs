...
pub struct StartOptions {
    ...
    pub backend: crate::esbuild::Backend,
}
...
async fn prepare_single(
    ...
    let split_sdk = split::detect_split_build(&project.root, &project.main_output()?);
    if split_sdk {
        tracing::info!("split-sdk build detected, watching in split mode");
    }
...
async fn prepare_members(
    ...
    for (i, project) in workspace.projects.iter().enumerate() {
        ...
        if split_sdk {
            tracing::info!(
                "split-sdk build detected in {}, watching in split mode",
