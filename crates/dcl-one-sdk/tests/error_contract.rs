...
#[test]
fn g12_bad_env_esbuild() {
    let f = Fixture::new("g12");
    f.write("scene.json", SCENE_OK);
    f.write("tsconfig.json", TSCONFIG);
    f.mkdir("node_modules/@dcl/sdk");
    let missing = f.path().join("no-such-esbuild").display().to_string();
    let envs = [
        ("DCL_ONE_SDK_ESBUILD", missing.as_str()),
        ("DCL_ONE_SDK_NO_SERVICE", "1"),
    ];
    let dir = f.dir_arg();
    let args = ["build", "--dir", &dir];
    let out = run(&args, &envs);
    assert_contract(
        &out,
        f.path(),
        "DCL_ONE_SDK_ESBUILD points at a missing file",
    );
    assert!(stderr_of(&out).contains("unset DCL_ONE_SDK_ESBUILD"));
    assert_verbose_chain(&args, &envs);
}
...
fn g30_rolldown_backend_needs_the_feature() {
    ...
    f.mkdir("node_modules/@dcl/sdk");
    let dir = f.dir_arg();
    let args = ["build", "--dir", &dir, "--backend", "rolldown"];
    let out = run(&args, &[]);
