...
//!   stdout is deliberately not captured: its step lines carry elapsed times
//!   and must never reach a golden.
//! * RUNTIME — `scripts/golden-runtime.mjs` loads the built scene in a node
//!   sandbox over a mocked `~system/*` table and runs upstream's short frame
//!   loop; its stdout is appended verbatim. The scene's own
//!   `CONSOLE(<level>):` lines are the only thing that catches a scene throwing
//!   on startup: the generated entrypoint catches everything `main()` throws
//!   and reports it with `console.error`, so the run still exits zero and can
//!   emit byte-identical CRDT traffic.
//!
//! No CPU metric: upstream's opcode and malloc counters come from a patched
//! QuickJS published only as an npm package, and this toolchain has no npm step
//! by design. The cost axis is exact per-artifact bytes, deploy total, per-run
//! CRDT message/byte counts, host-call counts and console output.
//!
//! Stale golden: `UPDATE_GOLDEN=1 cargo test -p dcl-one-sdk --test golden`, or
//! scripts/update-goldens.sh. A MISSING golden fails too — this repo's CI
//! backstop counts passing tests and never checks for a dirty tree, so an
//! auto-written golden would report green having asserted nothing.
...
/// Pinned in Cargo.toml as `rolldown = "=1.2.6"`. Repeated here so a bump has
/// to be copied into this line, invalidating every golden in one visible place
/// instead of scattering unexplained byte diffs.
...
/// Relative to the crate root and resolved through CARGO_MANIFEST_DIR, not
/// cwd: `cargo test` runs from the workspace root and `cargo test -p` from
/// wherever the caller stood.
...
/// Reuses the crdt encoder's own reference composite, so a change to it shows
/// up in both suites rather than only in the one nobody remembers to update.
...
/// dev-vs-prod is a build FLAG, not a property of a scene, so the second
/// snapshot costs one extra golden rather than a directory.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
...
/// parallel and every one calls this before touching anything else, so the
/// environment scrub below lands before any build reads it.
...
/// Keeps the per-pid paths from accumulating a toolchain per run. Only where
/// /proc can answer whether a pid is alive; elsewhere the trees stay until
/// `cargo clean`.
...
/// Blank lines stay blank rather than becoming two spaces: trailing whitespace
/// is the first thing an editor or a pre-commit hook silently eats, and a
/// golden it can edit is a golden that fails for no reason.
...
/// Dev bundles carry the absolute scene root inside the sourcemap's base64
/// `sourcesContent`, so hashing or sizing the raw file would make the golden a
/// function of where the temp dir landed.
...
/// stack traces carry it too; `<SCENE>` is what makes the text portable between
/// a worktree, a checkout and CI.
